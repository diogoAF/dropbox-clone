class DropBoxController {
            ...
            promises.push(
                new Promise((resolve, reject) => {
                    ...
                    if(file.type === 'folder') {
                        ...
                        this.removeFolder(this.currentFolder.join('|'), file.name, key).then(() => {
                            resolve({
                                fields: {
                                    key
                                }
    ...
    removeFolder(ref, name, key) {
        return new Promise((resolve, reject) => {
            ...
            let snapShotRef = folderRef.onSnapshot( snapshot => {
                snapshot.forEach(item => {
                    let data = item.data();
                    data.key = item.id;

                    if(data.type === 'folder') {
                        this.removeFolder(ref + '|' + name, data.name, data.key).then(() => {
                            resolve({
                                fields: {
                                    key: data.key
                                }
            ...
            this.getFirebaseRef().doc(key).delete().then(function() {
                console.log("Document successfully deleted!");
            }).catch(function(error) {
                console.error("Error removing document: ", error);
            });
    ...
    uploadProgress(event, file){
        ...
        this.timeLeftEl.innerHTML = this.formatTimeToReadable(timeLeft);
    }

    formatTimeToReadable(duration){
    ...
    readFiles(){
        ...
        this.cancelFolderSnapshotListener = this.getFirebaseRef().onSnapshot( snapshot => {
            ...
            snapshot.forEach(snapshotItem => {
                ...
                let key = snapshotItem.id;
                let data = snapshotItem.data();
